...
package Alr.Dependency_Graphs is
   ...
   function From_Instance (Instance : Alire.Containers.Release_Map) return Graph;
   ...
   function Filtering_Unused (This     : Graph;
                              Instance : Alire.Containers.Release_Map) return Graph;
   --  Remove dependencies that don't appear in the solution
   
   function Has_Dependencies (This : Graph;
                              Project : Alire.Project) return Boolean;
   --  Say if Project has dependencies in the current graph (hence not installable yet)
