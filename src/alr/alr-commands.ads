...
package Alr.Commands is
   ...
   --
   --  5. DESCRIPTION, custom detailed command explanation (optional).
   --  Commands can provide this text with Display_Help_Details (below).

   procedure Display_Help_Details (Cmd : Command) is null;
   --  Commands can display free text in this call. The text is printed after
   --  all other help texts. Corresponds to point 5 in the help summary.
