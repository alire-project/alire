...
package body Alr.Self is
   ...
   function Is_Canonical return Boolean is
      (Is_Rolling and then Src_Folder = Platform.Config_Folder / "alr");
   ...
   begin
      return
        Simple_Name (Parent (Platform.Own_Executable)) = "bin" and then
        Exists (Parent (Parent (Platform.Own_Executable)) / "alr_env.gpr");
   exception
      when E : others =>
   ...
   begin
      if Session.Alr_Src_Folder /= "" then
         ...
         return Parent (Parent (Platform.Own_Executable));
      else
