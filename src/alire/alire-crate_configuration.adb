...
package body Alire.Crate_Configuration is
            ...
            if Rel.Origin.Kind /= Alire.Origins.External then
               ...
               begin
                  if not Ent.Disabled then
                     ...
                     if Ent.Generate_Shell then
                        This.Generate_Shell_Config
                          (Rel.Name,
                           Conf_Dir / (+Rel.Name & "_config.sh"),
                           Version_Str);
                     end if;
                  end if;
               end;
   ...
   begin
      ...
      for Elt of Host_Info loop
         TIO.Put_Line (File,
                       Elt.Type_Def.Element.To_Shell_Declaration (Elt.Value));
      end loop;
      ...
      for C in This.Map.Iterate loop
         ...
         begin
            ...
            if AAA.Strings.Has_Prefix (Key, +Crate & ".") then
               TIO.New_Line (File);
               TIO.Put_Line (File, Type_Def.To_Shell_Declaration (Elt.Value));
            end if;
         end;
      end loop;
      TIO.New_Line (File);
      TIO.Close (File);
   end Generate_Shell_Config;
