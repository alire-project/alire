...
package body Alire.Roots is
   ...
   is
      ...
      begin
         ...
         if not Build_Root and then State.Crate = Name (This) then
            Trace.Debug ("Skipping build of root release as requested");
            return;
         end if;
   ...
   is
      use AAA.Strings;
      use Directories.Operators;
      Targets : Releases.Containers.Release_Map := This.Solution.Releases;
   begin
      ...
      --  Add root release when requested

      if With_Root then
         Targets.Insert (This.Release.Element.Name, This.Release.Element);
      end if;
      ...
      for Rel of Targets loop
         ...
         --  Config projects, being abstract, need no installation (I think).

         for Gpr_File of Rel.Project_Files
           (This.Environment, With_Path => True)
         loop
            declare
            ...
            begin
               if GNAT.OS_Lib.Is_Regular_File (Gpr_Path) then
                  Put_Info ("Installing " & Rel.Milestone.TTY_Image & "/"
                            & TTY.URL (Gpr_File) & "...");
                  Spawn.Gprinstall
                    (Project_File => Ada.Directories.Full_Name (Gpr_Path),
                     Prefix       => Prefix,
                     Recursive    => False,
                     Quiet        => True,
                     Extra_Args   => Cmd_Args);
               else
                  --  Binary crates may not include a GPR file, that we would
                  --  need to install its artifacts. This may be common for
                  --  compiler releases, so no need to be exceedingly alarmist
                  --  about it.
                  Trace.Log
                    ("Skipping installation of missing project: " & Gpr_Path,
      ...
      Put_Success ("Install to " & TTY.URL (Prefix)
