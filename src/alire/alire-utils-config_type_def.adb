...
package body Alire.Utils.Config_Type_Def is
   ...
   is
      From    : constant TOML_Adapters.Key_Queue :=
        TOML_Adapters.From (Raw, Key & "." & Name);
      Typ     : TOML_Value;
      First   : TOML_Value;
      Last    : TOML_Value;
   ...
   begin
      if Raw.Kind /= TOML_Table then
         From.Checked_Error (Key & " definition must be a table");
      end if;
      ...
      begin
         ...
         elsif Default_Required then
            From.Checked_Error ("default value required " & Key);
         else
