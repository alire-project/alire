...
### Built-in tests must be declared with `pragma Alire_Test`

With the built-in test runner, a `.adb` under the test crate's `src/` is now
run as a test only if it is a parameterless main procedure that declares itself
...
`separate` subunits, ...) are ignored silently, so they no longer need any
annotation. A main procedure that can run a test must declare itself; one that
is a helper main can opt out instead with:

```ada
pragma Alire_Test (Auxiliary_File);
```

An undeclared main procedure, or an `Alire_Test` pragma on something that is not
a parameterless main, is reported as a recoverable error (it fails `alr test`
unless `--force` is given). `Auxiliary_File` must be the only `Alire_Test`
configuration in its source.

This is a behavioral change: previously, every `.adb` under `src/` was run as a
test. Existing test sources must add `pragma Alire_Test;` (the skeleton
generated by `alr init` now does so).
