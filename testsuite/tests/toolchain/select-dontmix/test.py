...
p = run_alr("default-toolchain", "--select", "gnat_native", "gprbuild/=8888",
            complain_on_error=False)
assert_match(".*Use --force to override compatibility checks.*", p.out)

# But leaving free choice of gprbuild will result in the native being chosen
run_alr("default-toolchain", "--select", "gnat_native", "gprbuild")
p = run_alr("default-toolchain")
...
p = run_alr("default-toolchain", "--select", "gnat_external", complain_on_error=False)
...
run_alr("default-toolchain", "--select", "gnat_external", force=True)
run_alr("default-toolchain", "--select", "gprbuild")
p = run_alr("default-toolchain")
...
run_alr("default-toolchain", "--select", "gprbuild", "gnat")
p = run_alr("default-toolchain")
assert_match(".*gprbuild      8888.0.0 Default.*", p.out)
assert_match(".*gnat_native   8888.0.0 Default.*", p.out)
assert_match(".*gnat_external [\d\s.]+ Available.*", p.out)
