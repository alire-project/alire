...
class DockerWrapperDriver(BaseDriver):
    ...
    def run(self):
        ...
        try:
            ...
            self.result.log += f'Docker command failed with exception: {e}'
            # Also add traceback:
            self.result.log += f'\nTraceback:\n{e.with_traceback()}'
            ...
            # Ensure output was generated by checking output existence
            if 'output' not in locals():
                self.result.log += "Docker command did not generate any output"
                raise TestAbortWithFailure("Docker command did not generate any output")
